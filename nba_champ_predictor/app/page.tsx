import React from 'react'
import Image from 'next/image'
import { Menu, MenuButton, MenuItem, MenuItems } from '@headlessui/react'
import { ChevronDownIcon } from '@heroicons/react/20/solid'
import Atlanta from '../public/img/Atlanta_Hawks_logo.svg.png'
import Boston from '../public/img/Boston_Celtics.svg.png'
import Brooklyn from '../public/img/Brooklyn_Nets_primary_icon_logo_2024.svg.png'
import Charlotte from '../public/img/Charlotte_Hornets_(2014).svg.png'
import Chicago from '../public/img/Chicago_Bulls_logo.svg.png'
import Cleveland from '../public/img/Cleveland_Cavaliers_logo.svg.png'
import Dallas from '../public/img/Dallas_Mavericks_logo.svg.png'
import Denver from '../public/img/Denver_Nuggets.svg.png'
import Warriors from '../public/img/Golden_State_Warriors_logo.svg.png'
import Houston from '../public/img/Houston_Rockets.svg.png'
import Indiana from '../public/img/Indiana_Pacers.svg.png'
import Detroit from '../public/img/Logo_of_the_Detroit_Pistons.svg.png'
import Clippers from '../public/img/Los_Angeles_Clippers_(2024).svg.png'
import Lakers from '../public/img/Los_Angeles_Lakers_logo.svg.png'
import Memphis from '../public/img/Memphis_Grizzlies.svg.png'
import Miami from '../public/img/Miami_Heat_logo.svg.png'
import Milwaukee from '../public/img/Milwaukee_Bucks_logo.svg.png'
import Minnesota from '../public/img/Minnesota_Timberwolves_logo.svg.png'
import Pelicans from '../public/img/New_Orleans_Pelicans_logo.svg.png'
import Knicks from '../public/img/New_York_Knicks_logo.svg.png'
import Thunder from '../public/img/Oklahoma_City_Thunder.svg.png'
import Orlando from '../public/img/Orlando_Magic_logo.svg.png'
import Philadelphia from '../public/img/Philadelphia_76ers_logo.svg.png'
import Phoenix from '../public/img/Phoenix_Suns_logo.svg.png'
import Portland from '../public/img/Portland_Trail_Blazers_logo.svg.png'
import Sacramento from '../public/img/SacramentoKings.svg.png'
import Spurs from '../public/img/San_Antonio_Spurs.svg.png'
import Toronto from '../public/img/Toronto_Raptors_logo.svg.png'
import Utah from '../public/img/Utah_Jazz_logo_2025.svg.png'
import Washington from '../public/img/Washington_Wizards_logo.svg.png'

export default function Home() {
  return (
    <div className="
      bg-gradient-to-b 
      from-orange-500
      via-pink-500
      to-blue-500
      h-screen w-full 
      flex items-center justify-center 
      text-white
    ">
      <div className="content-center text-center text-white">
        <div className="text-4xl font-bold m-5 animate-appear">Hi, I'm (insert app name here).</div>
        <div className="text-base animate-appear m-5">I am an AI trained on advanced NBA statistics spanning from the 1996 season.
          Give me a season to predict and I'll tell you who think won (or will win)!
        </div>

        <Menu as="div" className="relative inline-block text-left m-5">
          <div>
            <MenuButton className="inline-flex w-full justify-center gap-x-1.5 rounded-md bg-white px-3 py-2 text-sm font-semibold text-gray-900 shadow-xs ring-1 ring-gray-300 ring-inset hover:bg-gray-50">
              Select Season:
              <ChevronDownIcon aria-hidden="true" className="-mr-1 size-5 text-gray-400" />
            </MenuButton>
          </div>

          <MenuItems
            transition
            className="absolute right-0 z-10 mt-2 w-25 origin-top-right overflow-auto h-40 rounded-md bg-white shadow-lg ring-1 ring-black/5 transition focus:outline-hidden data-closed:scale-95 data-closed:transform data-closed:opacity-0 data-enter:duration-100 data-enter:ease-out data-leave:duration-75 data-leave:ease-in"
          >
            <div className="py-1">
              <MenuItem>
                <a
                  href="#"
                  className="block px-4 py-2 text-sm text-gray-700 data-focus:bg-gray-100 data-focus:text-gray-900 data-focus:outline-hidden"
                >
                  1996-97
                </a>
              </MenuItem>
              <MenuItem>
                <a
                  href="#"
                  className="block px-4 py-2 text-sm text-gray-700 data-focus:bg-gray-100 data-focus:text-gray-900 data-focus:outline-hidden"
                >
                  1997-98
                </a>
              </MenuItem>
              <MenuItem>
                <a
                  href="#"
                  className="block px-4 py-2 text-sm text-gray-700 data-focus:bg-gray-100 data-focus:text-gray-900 data-focus:outline-hidden"
                >
                  1998-99
                </a>
              </MenuItem>
              <MenuItem>
                <a
                  href="#"
                  className="block px-4 py-2 text-sm text-gray-700 data-focus:bg-gray-100 data-focus:text-gray-900 data-focus:outline-hidden"
                >
                  1999-00
                </a>
              </MenuItem>
              <MenuItem>
                <a
                  href="#"
                  className="block px-4 py-2 text-sm text-gray-700 data-focus:bg-gray-100 data-focus:text-gray-900 data-focus:outline-hidden"
                >
                  2000-01
                </a>
              </MenuItem>
              <MenuItem>
                <a
                  href="#"
                  className="block px-4 py-2 text-sm text-gray-700 data-focus:bg-gray-100 data-focus:text-gray-900 data-focus:outline-hidden"
                >
                  2001-02
                </a>
              </MenuItem>
              <MenuItem>
                <a
                  href="#"
                  className="block px-4 py-2 text-sm text-gray-700 data-focus:bg-gray-100 data-focus:text-gray-900 data-focus:outline-hidden"
                >
                  2002-03
                </a>
              </MenuItem>
              <MenuItem>
                <a
                  href="#"
                  className="block px-4 py-2 text-sm text-gray-700 data-focus:bg-gray-100 data-focus:text-gray-900 data-focus:outline-hidden"
                >
                  2003-04
                </a>
              </MenuItem>
              <MenuItem>
                <a
                  href="#"
                  className="block px-4 py-2 text-sm text-gray-700 data-focus:bg-gray-100 data-focus:text-gray-900 data-focus:outline-hidden"
                >
                  2004-05
                </a>
              </MenuItem>
              <MenuItem>
                <a
                  href="#"
                  className="block px-4 py-2 text-sm text-gray-700 data-focus:bg-gray-100 data-focus:text-gray-900 data-focus:outline-hidden"
                >
                  2005-06
                </a>
              </MenuItem>
              <MenuItem>
                <a
                  href="#"
                  className="block px-4 py-2 text-sm text-gray-700 data-focus:bg-gray-100 data-focus:text-gray-900 data-focus:outline-hidden"
                >
                  2006-07
                </a>
              </MenuItem>
              <MenuItem>
                <a
                  href="#"
                  className="block px-4 py-2 text-sm text-gray-700 data-focus:bg-gray-100 data-focus:text-gray-900 data-focus:outline-hidden"
                >
                  2007-08
                </a>
              </MenuItem>
              <MenuItem>
                <a
                  href="#"
                  className="block px-4 py-2 text-sm text-gray-700 data-focus:bg-gray-100 data-focus:text-gray-900 data-focus:outline-hidden"
                >
                  2008-09
                </a>
              </MenuItem>
              <MenuItem>
                <a
                  href="#"
                  className="block px-4 py-2 text-sm text-gray-700 data-focus:bg-gray-100 data-focus:text-gray-900 data-focus:outline-hidden"
                >
                  2009-10
                </a>
              </MenuItem>
              <MenuItem>
                <a
                  href="#"
                  className="block px-4 py-2 text-sm text-gray-700 data-focus:bg-gray-100 data-focus:text-gray-900 data-focus:outline-hidden"
                >
                  2010-11
                </a>
              </MenuItem>
              <MenuItem>
                <a
                  href="#"
                  className="block px-4 py-2 text-sm text-gray-700 data-focus:bg-gray-100 data-focus:text-gray-900 data-focus:outline-hidden"
                >
                  2011-12
                </a>
              </MenuItem>
              <MenuItem>
                <a
                  href="#"
                  className="block px-4 py-2 text-sm text-gray-700 data-focus:bg-gray-100 data-focus:text-gray-900 data-focus:outline-hidden"
                >
                  2012-13
                </a>
              </MenuItem>
              <MenuItem>
                <a
                  href="#"
                  className="block px-4 py-2 text-sm text-gray-700 data-focus:bg-gray-100 data-focus:text-gray-900 data-focus:outline-hidden"
                >
                  2013-14
                </a>
              </MenuItem>
              <MenuItem>
                <a
                  href="#"
                  className="block px-4 py-2 text-sm text-gray-700 data-focus:bg-gray-100 data-focus:text-gray-900 data-focus:outline-hidden"
                >
                  2014-15
                </a>
              </MenuItem>
              <MenuItem>
                <a
                  href="#"
                  className="block px-4 py-2 text-sm text-gray-700 data-focus:bg-gray-100 data-focus:text-gray-900 data-focus:outline-hidden"
                >
                  2015-16
                </a>
              </MenuItem>
              <MenuItem>
                <a
                  href="#"
                  className="block px-4 py-2 text-sm text-gray-700 data-focus:bg-gray-100 data-focus:text-gray-900 data-focus:outline-hidden"
                >
                  2016-17
                </a>
              </MenuItem>
              <MenuItem>
                <a
                  href="#"
                  className="block px-4 py-2 text-sm text-gray-700 data-focus:bg-gray-100 data-focus:text-gray-900 data-focus:outline-hidden"
                >
                  2017-18
                </a>
              </MenuItem>
              <MenuItem>
                <a
                  href="#"
                  className="block px-4 py-2 text-sm text-gray-700 data-focus:bg-gray-100 data-focus:text-gray-900 data-focus:outline-hidden"
                >
                  2018-19
                </a>
              </MenuItem>
              <MenuItem>
                <a
                  href="#"
                  className="block px-4 py-2 text-sm text-gray-700 data-focus:bg-gray-100 data-focus:text-gray-900 data-focus:outline-hidden"
                >
                  2019-20
                </a>
              </MenuItem>
              <MenuItem>
                <a
                  href="#"
                  className="block px-4 py-2 text-sm text-gray-700 data-focus:bg-gray-100 data-focus:text-gray-900 data-focus:outline-hidden"
                >
                  2020-21
                </a>
              </MenuItem>
              <MenuItem>
                <a
                  href="#"
                  className="block px-4 py-2 text-sm text-gray-700 data-focus:bg-gray-100 data-focus:text-gray-900 data-focus:outline-hidden"
                >
                  2021-22
                </a>
              </MenuItem>
              <MenuItem>
                <a
                  href="#"
                  className="block px-4 py-2 text-sm text-gray-700 data-focus:bg-gray-100 data-focus:text-gray-900 data-focus:outline-hidden"
                >
                  2022-23
                </a>
              </MenuItem>
              <MenuItem>
                <a
                  href="#"
                  className="block px-4 py-2 text-sm text-gray-700 data-focus:bg-gray-100 data-focus:text-gray-900 data-focus:outline-hidden"
                >
                  2023-24
                </a>
              </MenuItem>
              <MenuItem>
                <a
                  href="#"
                  className="block px-4 py-2 text-sm text-gray-700 data-focus:bg-gray-100 data-focus:text-gray-900 data-focus:outline-hidden"
                >
                  2024-25
                </a>
              </MenuItem>
            </div>
          </MenuItems>
        </Menu>
        <div className='flex mt-10 gap-x-3 overflow-hidden animate-infinite-scroll'>
          <Image src={Atlanta} alt='' width={80} height={60}/>
          <Image src={Boston} alt='' width={70} height={60}/>
          <Image src={Brooklyn} alt='' width={80} height={60}/>
          <Image src={Charlotte} alt='' width={80} height={60}/>
          <Image src={Chicago} alt='' width={80} height={60}/>
          <Image src={Cleveland} alt='' width={70} height={60}/>
          <Image src={Dallas} alt='' width={80} height={60}/>
          <Image src={Denver} alt='' width={70} height={60}/>
          <Image src={Detroit} alt='' width={80} height={60}/>
          <Image src={Warriors} alt='' width={70} height={60}/>
          <Image src={Houston} alt='' width={70} height={60}/>
          <Image src={Indiana} alt='' width={80} height={60}/>
          <Image src={Clippers} alt='' width={90} height={60}/>
          <Image src={Lakers} alt='' width={120} height={60}/>
          <Image src={Memphis} alt='' width={70} height={60}/>
          <Image src={Miami} alt='' width={70} height={60}/>
          <Image src={Milwaukee} alt='' width={70} height={60}/>
          <Image src={Minnesota} alt='' width={70} height={60}/>
          <Image src={Pelicans} alt='' width={70} height={60}/>
          <Image src={Knicks} alt='' width={70} height={60}/>
          <Image src={Thunder} alt='' width={70} height={60}/>
          <Image src={Orlando} alt='' width={70} height={60}/>
          <Image src={Philadelphia} alt='' width={70} height={60}/>
          <Image src={Phoenix} alt='' width={70} height={60}/>
          <Image src={Portland} alt='' width={70} height={60}/>
          <Image src={Sacramento} alt='' width={70} height={60}/>
          <Image src={Spurs} alt='' width={70} height={60}/>
          <Image src={Toronto} alt='' width={70} height={60}/>
          <Image src={Utah} alt='' width={70} height={60}/>
          <Image src={Washington} alt='' width={70} height={60}/>
        </div>
      </div>
    </div>
  );
}


